...
# fallback to legacy path for in-flight processes
[ ! -f "$PID_FILE" ] && PID_FILE="$ROOT/03-system/logs/autonomous_loop.pid"
if [ -f "$PID_FILE" ]; then
  LOOP_PID=$(cat "$PID_FILE")
  if kill -0 "$LOOP_PID" 2>/dev/null; then
    echo "| Autonomous Loop | UP | PID $LOOP_PID (system/logs/autonomous_loop.pid) |" >> /tmp/state.md
  else
