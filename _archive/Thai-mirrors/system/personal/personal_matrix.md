...
This directory governs the corporate-style division of labor for เปรม's Personal Empire (`system/personal/`). It maps **28 Personal Agents** into 10 strict functional pillars, guiding autonomous task delegation, power-center veto gates, and information routing.
...
These are non-negotiable rules that define which pillar holds **ultimate authority** over each project domain. Power Center sign-off is **mandatory** before any deliverable is presented to เปรม.
...
4. ── POWER CENTER GATE ──────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────────
        ↓ Vetting Logic Gate:
        - จำกัดระดับความขัดแย้งให้เกิด HARD REJECT ได้สะสมสูงสุดไม่เกิน 2 ครั้งต่อหนึ่งงาน
        - ในความขัดแย้งครั้งที่ 3 ให้ทำการระงับงานนั้น ดึงข้อมูลจุดขัดแย้ง (Impasse) ออกมา 
          และทำการส่งคำร้องตรงไปที่ "หมวดคำสั่งผู้บริหาร (Executive Command)" บนหน้า Master Dashboard
5. Jeff Compiles Executive OKR Dashboard Report
   → Includes Power Center Vetting Log as MANDATORY section
       ↓ HITL Screen
6. เปรม — Reviews and types Approved / Reject
       ↓ Approved
